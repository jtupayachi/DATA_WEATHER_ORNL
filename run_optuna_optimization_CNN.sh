#!/bin/bash
# ==================== Bayesian Hyperparameter Optimization Script - CNN/TCN ====================
# This script runs Optuna-based Bayesian optimization for CNN and TCN models
# Optimizes for MCC (Matthews Correlation Coefficient)
#
# Usage:
#   ./run_optuna_optimization_CNN.sh              # Run with defaults (CNN, 50 trials)
#   ./run_optuna_optimization_CNN.sh --model tcn  # Run TCN only
#   ./run_optuna_optimization_CNN.sh --model both # Run both CNN and TCN
#   ./run_optuna_optimization_CNN.sh --n_trials 100 --gpu 1  # Custom settings
#
# ====================================================================================
...
MODEL="both"           # Options: cnn, tcn, both
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Options:"
            echo "  --model MODEL       Model type: cnn, tcn, or both (default: both)"
            ...
            echo ""
            echo "Example:"
            echo "  $0 --model tcn --n_trials 100 --gpu 0"
            exit 0
            ;;
        *)
...
echo ""
echo "=========================================="
echo "Optimization Configuration (CNN/TCN)"
echo "=========================================="
...
# ==================== SEARCH SPACE INFO ====================
echo "Search Space:"
echo "  - sequence_length: [24, 48, 96]"
echo "  - batch_size: [16, 32, 64]"
echo "  - hidden_dim: [64, 128, 256]"
echo "  - num_layers: [2, 3, 4, 5]"
echo "  - kernel_size: [3, 5, 7]"
echo "  - dropout: [0.1, 0.5]"
echo "  - learning_rate: [1e-4, 1e-2]"
echo "  - optimizer: [adam, adamw]"
echo "  - scheduler: [plateau, cosine, none]"
echo ""
...
echo ""
echo "=========================================="
echo "Starting Bayesian Optimization (CNN/TCN)..."
echo "=========================================="
echo "Start time: $(date)"
echo ""

# Build command
CMD="python3 JT_Extreme_event_labeler_CNN_optuna.py"
CMD="$CMD --model $MODEL"
...
LOG_FILE="optuna_cnn_optimization_${MODEL}_${TIMESTAMP}.log"
...
RESULTS_DIR=$(ls -td optuna_cnn_results_* 2>/dev/null | head -1)
...
echo "3. Run training with best params using the main CNN script"
echo ""
