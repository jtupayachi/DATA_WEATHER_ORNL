    ...
    for path in possible_paths:
        if os.path.exists(path):
            ...
            with open(path, 'r') as f:
                data = json.load(f)
            return data['tower_coordinates'], data['region_info']
...
TOWER_COORDINATES, REGION_INFO_FROM_JSON = load_tower_metadata()
...
# Region info from JSON (with fallback for compatibility)
REGION_INFO = {
    'name': REGION_INFO_FROM_JSON.get('name', 'Oak Ridge Reservation'),
    'state': REGION_INFO_FROM_JSON.get('state', 'Tennessee, USA'),
    'center_lat': REGION_INFO_FROM_JSON.get('center_lat', 35.928252),
    'center_lon': REGION_INFO_FROM_JSON.get('center_lon', -84.317857),
    'terrain': REGION_INFO_FROM_JSON.get('terrain', 'Ridge and Valley Province, Appalachian Region'),
}
